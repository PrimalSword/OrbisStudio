...
def build_delta_manifest(old_root: Path, new_root: Path) -> DeltaManifest:
    ...
    for name in sorted(old_files.keys() | new_files.keys()):
        ...
        if old is None:
            if new is None:
                raise RuntimeError(f"internal OTA manifest error for {name}")
            entries.append(
                DeltaEntry(name, "added", None, _digest(new), None, new.stat().st_size)
            )
        elif new is None:
            entries.append(
                DeltaEntry(name, "removed", _digest(old), None, old.stat().st_size, None)
            )
        else:
